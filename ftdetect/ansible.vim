function! s:isAnsible()
  ...
  if filepath =~? '\v/(provisioning|tasks|roles|handlers)/.*\.ya?ml$' | return 1 | en
  ...
  if exists('g:ansible_template_syntaxes')
    ...
    for syntax_name in items(g:ansible_template_syntaxes)
      ...
      if filepath =~? s:syntax_string
        execute 'set filetype='syntax_name[1].'.jinja2'
        return
      endif
    endfor
  endif
...
augroup ansible_vim_filetypeyaml_ansible
    au!
    au BufNewFile,BufRead * if s:isAnsible() | set filetype=yaml.ansible | en
augroup END
augroup ansible_vim_filetypejinja2
    au!
    au BufNewFile,BufRead *.j2 call s:setupTemplate()
augroup END
augroup ansible_vim_filetypehosts
    au!
    au BufNewFile,BufRead hosts set filetype=ansible_hosts
augroup END.
